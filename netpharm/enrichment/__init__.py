...
from .david import DavidEnrichmentClient
...
class EnrichmentAnalyzer:
    ...
    def run_david_service(self, gene_list, output_dir, email):
        """
        Run automated DAVID analysis via SOAP service.
        """
        self.logger.info("\n" + "="*70)
        self.logger.info("[STEP 5] FUNCTIONAL ENRICHMENT ANALYSIS - DAVID (AUTOMATED)")
        self.logger.info("="*70)
        
        if not email:
            self.logger.error("❌ DAVID email required for automated analysis.")
            return None
            
        client = DavidEnrichmentClient(email)
        try:
            self.logger.info("Authenticating with DAVID Web Service...")
            client.authenticate()
            
            self.logger.info(f"Submitting {len(gene_list)} genes...")
            client.add_gene_list(gene_list, list_name="NetPharmPy_List")
            ...
            self.logger.info("Retrieving chart report...")
            results = client.get_chart_report(threshold=0.05, min_count=2)
            
            # Save results
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, "david_automated_results.csv")
            
            # Convert to DataFrame
            df = pd.DataFrame(results)
            df.to_csv(output_file, index=False)
            self.logger.info(f"✓ DAVID results saved to: {output_file}")
            
            return df
            
        except Exception as e:
            self.logger.error(f"❌ DAVID Analysis Failed: {str(e)}")
            return None
